...
export default function TpsChart({
	...
	history: { time: Date; tps: number }[];
}) {
	const containerRef = useRef<HTMLDivElement>(null);
	const [size] = useState({ w: 430, h: 200 });
	...
	if (history.length < 2) {
		return (
			<div className="flex items-center justify-center h-[150px] text-xs text-[var(--color-text-muted)] font-[var(--font-mono)]">
				Collecting data…
			</div>
		);
	}
	...
	const xLabels = [0, 0.25, 0.5, 0.75, 1].map((frac) => {
		...
		return {
			x: PAD.left + frac * iW,
			label: history[idx].time.toLocaleTimeString([], {
				hour: "2-digit",
				minute: "2-digit",
				second: "2-digit",
			}),
