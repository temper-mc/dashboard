...
export default function Sidebar({activePage, setActivePage}: SidebarProps) {
	return (<aside
		className="w-20 flex flex-col items-center py-6 gap-2 bg-bg-secondary border-r border-border relative z-10">
		{/* Logo mark */}
		<div className="mb-6 w-10 h-10 rounded-xl flex items-center justify-center"
		     style={{background: "linear-gradient(135deg, var(--color-info), var(--color-primary))"}}>
			<span className="text-white text-sm font-(--font-heading)">T</span>
		</div>
		...
		<div className="mt-auto w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_6px_2px_rgba(52,211,153,0.4)]"
		     title="Connected"/>
	</aside>);
}