...
class TelemetryStore {
	...
	private updateHandshake(data: any) {
		if (data.system) {
			if (data.system.cpu_model !== undefined) this._state.data.cpuModel = data.system.cpu_model;
			if (data.system.cpu_cores !== undefined) this._state.data.cpuCores = data.system.cpu_cores;
			if (data.system.cpu_threads !== undefined) this._state.data.cpuThreads = data.system.cpu_threads;
		}
		if (data.config) {
			if (data.config.max_players !== undefined) this._state.data.maxPlayers = data.config.max_players;
		}
	}

	private updateMetrics(data: any) {
		if (data.ram_usage !== undefined) this._state.data.ramUsage = data.ram_usage;
		if (data.total_ram !== undefined) this._state.data.totalRam = data.total_ram;
		if (data.cpu_usage !== undefined) this._state.data.cpuUsage = data.cpu_usage;
		if (data.uptime !== undefined) this._state.data.uptime = data.uptime;
		if (data.player_count !== undefined) this._state.data.onlinePlayers = data.player_count;
		if (data.online_players !== undefined) this._state.data.onlinePlayers = data.online_players;
		if (data.max_players !== undefined) this._state.data.maxPlayers = data.max_players;
		if (data.storage_used !== undefined) this._state.data.storageUsed = data.storage_used;
		if (data.tps !== undefined) {
			this._state.data.tps = data.tps;
			this._state.data.tpsHistory = [
				...this._state.data.tpsHistory.slice(-59),
